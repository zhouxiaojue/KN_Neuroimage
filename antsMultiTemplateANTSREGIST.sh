...
if [[ "$RIGID" -eq 1 ]];
  ...
  #she also prepends this:
      echo '#!/usr/bin/env bash' > $qscript
      echo "mkdir ants" >> $qscript
      echo "mv ./$ANTS ants/" >> $qscript
      echo "mv ./$WARP ants/" >> $qscript
      echo "mv ./$N4 ants/" >> $qscript
      echo "mv ./$PEXEC ants/" >> $qscript
      echo "mv ./$COND ants/" >> $qscript
      echo "chmod -R a=wrx ants" >> $qscript
      echo "export PATH=\$PWD/ants:\$PATH" >> $qscript
...
#NOTE WE WILL HAVE TO PUT THIS IN ANTSPATH NOT JUST CURRENT DIRECTORY
  /StudyDir/bin/waitForCONDORJobs_wrapper.pl 1 120 $jobIDs
      if [ ! $? -eq 0 ]; then
...
while [[ $i -lt ${ITERATIONLIMIT} ]];
  ...
  #BMN
            IMAGEcondor="$IMAGEcondor,${IMAGESETARRAY[$l]}"; #BMN I'm modifying this to just use a spaced string and implicit split instead of an array/hash
  ...
  #/BMN
      ...
      DEFORMEDcondor="$DEFORMEDcondor ${outdir}/${OUTFN}${l}WarpedToTemplate.nii.gz"; #This collects the outputs for this job I hope
    ...
    #\BMN
      ...
      pexe="$pexebase ${basecall} ${stage0} ${stage3} >> ${outdir}/job_${count}_metriclog.txt\n"
        ...
        qscript="${outdir}/job_${count}_${i}_${OUTWARPFN}.sh"
...
#BMN END OF NEW BLOCK
  ...
  #BMN JUST COPY EVERYTHING FOR NOW
          ...
          then
            ...
            mv ${outdir}/job*.output ${outdir}/ANTs_iteration_${i}
        fi
      else

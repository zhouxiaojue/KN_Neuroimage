...
#CONDORCHANGE
COND=${ANTSPATH}/waitForCONDORJobs_wrapper.pl
fle_error=0
...
Optional arguments:
     ...
     -c:  Control for parallel computation (default 1) -- 0 == run serially,  1 == SGE qsub,
          2 == use PEXEC (localhost), 3 == Apple XGrid, 4 == PBS qsub, 5 == SLURM, 6 == HTCCondor
...
while getopts "a:b:c:d:g:h:i:j:k:m:n:o:p:s:r:t:w:x:y:z:" OPT
  do
  case $OPT in
      h) #help
   echo "$USAGE"
   ...
   ;;
      a) #gaussian smooth in GR option (default = 3,0 )
   Gaussian=$OPTARG
   ;;
...
if [[ $DOQSUB -eq 6 ]];
  then
    ...
    if [[ ${#qq} -lt 1 ]];
      then
        echo "is you on condor submit node?  if not, then choose another c option ... "
        exit
      fi
  fi
...
##########################################################################
#
# perform rigid body registration if requested, if using condor, needs to be modified the variables in submit files and also qcondorscript 
#
##########################################################################
if [[ "$RIGID" -eq 1 ]];
    then
    ...
    for (( i = 0; i < ${#IMAGESETARRAY[@]}; i+=$NUMBEROFMODALITIES ))
        ...
        qcondorscript="${outdir}/job_${count}_qsub_condor.sh"
        rm -f $qscript
        rm -f $qcondorscript
        if [[ $DOQSUB -eq 5 ]];
            then
 ...
 ${ANTSPATH}/waitForSGEQJobs.pl 1 60 $jobIDs
    ...
    if [[ BACKUP_EACH_ITERATION -eq 1 ]];
      then
	    ...
	    mv ${outdir}/job*.output ${outdir}/rigid/
        #CONDOR CHANGE
        fi
      else
...
while [[ $i -lt ${ITERATIONLIMIT} ]];
    ...
    for (( j = 0; j < ${#IMAGESETARRAY[@]}; j+=$NUMBEROFMODALITIES ))
        ...
        for (( k = 0; k < $NUMBEROFMODALITIES; k++ ))
          do
            ...
            DEFORMEDCondor="${OUTFN}${l}WarpedToTemplate.nii.gz"
            ...
            if [[ $N4CORRECT -eq 1 ]];
              then
                ...
                execondor=" $execondor $N4 -d ${DIM} -b [200] -c [50x50x40x30,0.00000001] -i ${IMAGESETARRAY[$l]} -o ${REPAIRED} -r 0 -s 2\n"
                ...
                warpexecondor=" $warpexe WarpImageMultiTransform ${DIM} ${REPAIRED} ${DEFORMEDCondor} -R ${TEMPLATES[$k]} ${OUTWARPFN}Warp.nii.gz ${OUTWARPFN}Affine.txt\n"
                ...
                warpexecondor=" $warpexecondor WarpImageMultiTransform ${DIM} ${IMAGESETARRAY[$l]} ${DEFORMEDCondor} -R ${TEMPLATES[$k]} ${OUTWARPFN}Warp.nii.gz ${OUTWARPFN}Affine.txt\n"
                ...
                IMAGEcondor="${IMAGEcondor}${IMAGESETARRAY[$l]},"
                TEMPLATEcondor="${TEMPLATEcondor}${TEMPLATES[$k]},"
                DEFORMEDCondorSet="${DEFORMEDCondorSet}${DEFORMEDCondor},"
            fi
            #CONDOR CHANGE NEED TO import input files 
            #echo "TEMPLATESarray $TEMPLATEcondor current:${TEMPLATES[$k]}, IMAGESETall: $IMAGEcondor current: ${IMAGESETARRAY[$l]}"
            #CONDOR CHANGE
        done
        IMGbase=`basename ${IMAGESETARRAY[$j]}`
        POO=${OUTPUTNAME}${IMGbase} 
        ...
        execondor="$execondor ANTS ${DIM} $IMAGEMETRICSET -i ${MAXITERATIONS} -t ${TRANSFORMATION} -r $REGULARIZATION -o ${OUTWARPFN} --use-Histogram-Matching  $LINEARTRANSFORMPARAMS\n"
        execondor="$execondor $warpexecondor"
        ...
        qcondorscript="job_${count}_${i}_${OUTWARPFN}_condor.sh"
        ...
        if [[ $DOQSUB -eq 1 ]];
            ...
            then
            #DELETE
            echo "input is 6 condor"
            echo '#!/usr/bin/env bash' >> $qcondorscript
            echo "mkdir ants" >> $qcondorscript
            echo "mv ./N4BiasFieldCorrection ants/" >> $qcondorscript
            echo "mv ./ANTS ants/" >> $qcondorscript
            echo "mv ./WarpImageMultiTransform ants/" >> $qcondorscript
            echo "chmod -R a=wrx ants" >> $qcondorscript
            echo "export PATH=\$PWD/ants:\$PATH" >> $qcondorscript
            echo -e "$execondor" >> $qcondorscript
            echo "You chose to run on CONDOR.  Make sure this computer is a submit node!" ;
            chmod +x $qcondorscript ; ###THIS IS ESSENTIAL FOR CONDOR!
            echo -e "Universe = vanilla\ngetenv = True\nRequirements  = ( OpSys == \"LINUX\" && Arch == \"X86_64\" )\nArguments = \nPeriodicRemove =  (((CurrentTime - EnteredCurrentStatus) > (2*24*3600)) && JobStatus == 5)\n\ninitialdir = $PWD\nExecutable = $qcondorscript \nshould_transfer_files = yes \nwhen_to_transfer_output = ON_EXIT \ntransfer_input_files = /StudyDir/bin/ANTS,/StudyDir/bin/ImageMath,/StudyDir/bin/N4BiasFieldCorrection,/StudyDir/bin/WarpImageMultiTransform,/StudyDir/bin/ANTSpexec.sh,$IMAGEcondor $TEMPLATEcondor \ntransfer_output_files = ${OUTWARPFN}Warp.nii.gz,${OUTWARPFN}Affine.txt,${OUTWARPFN}InverseWarp.nii.gz,${DEFORMEDCondorSet} \nrequest_disk = 6GB\nrequest_memory = 8GB\nLog = ${qcondorscript}.log\nError = ${qcondorscript}.err\nOutput = ${qcondorscript}.output\nQueue\n" > ${qcondorscript}.submit;
            id=`condor_submit ${qcondorscript}.submit | awk '{print $6}'`0 ;
            echo "Debug: adding job ID $id"
            jobIDs="$jobIDs $id"
            #CONDOR CHANGE
            IMAGEcondor=""
    ...
    for (( j = 0; j < $NUMBEROFMODALITIES; j++ ))
        do
        echo "shapeupdatetotemplate ${DIM} ${TEMPLATES[$j]} ${TEMPLATENAME} ${OUTPUTNAME} ${GRADIENTSTEP} ${j}"

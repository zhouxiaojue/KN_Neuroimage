...
#TIME TO GET RID OF THIS, DOES NOT WORK ON CONDOR
#ANTS=antsRegistration
ANTSPATH=/StudyDir/bin
ANTS=ANTS
...
fle_error=0
#BMN WAS CAUSING ISSUES WITH THE NON CONDOR JOBS
COND=waitForCONDORJobs_wrapper.pl
for FLE in $ANTS $WARP $N4 $PEXEC $COND 
#for FLE in $ANTS $WARP $N4 $PEXEC $COND $SGE $XGRID $PBS $SLURM
#BMN END OF NEW BLOCK
  do
    if [ ! -s ./$FLE ] ; then
	#BMN we'll copy them to the local directory so they're easy to transfer on CHTC
	cp $ANTSPATH/$FLE . ;
    else
	#oops, need to overwrite these since the syncing of the folder causes the linux binaries to be copied locally
	cp $ANTSPATH/$FLE . ;
...
# This keeps the template shape stable over multiple iterations of template building
    ...
    #FAIL - this is too broad, why aren't we getting the exact templatename (with the trailing index, e.g. 0, 1, etc) to capture only those from this modality??
    #somehow this is getting much more than just the 0th set
#    imagelist=(`ls ${outputname}template${whichtemplate}*WarpedToTemplate.nii.gz`)
    imagelist=(`for file in ${outputname}template${whichtemplate}*WarpedToTemplate.nii.gz; do [ -s $file ]&&echo -n "$file "; done `)
    if [[ ${#imagelist[@]} -eq 0  ]] ; then
      echo ERROR shapeupdatedtotemplate - imagelist length is 0
      exit 1
    fi

    echo "DEBUG: summarizeimageset $dim $template $statsmethod ${imagelist[@]}  "
    summarizeimageset $dim $template $statsmethod ${imagelist[@]}

    #THIS NEEDS TO BE MORE FOCUSED TOO
    #WARPLIST=( `ls ${outputname}*[0-9]Warp.nii.gz 2> /dev/null` )
    WARPLIST=(`for file in ${outputname}*[0-9]Warp.nii.gz ; do [ -s $file ]&&echo -n "$file "; done `)
    NWARPS=${#WARPLIST[*]}
    ...
    if [[ $whichtemplate -eq 0 ]];
      then
        ...
        if [[ $NWARPS -ne 0 ]]; then
          ...
          echo "   ${ANTSPATH}/AverageImages $dim ${templatename}${whichtemplate}warp.nii.gz 0 `for file in ${outputname}*Warp.nii.gz ; do [ -s $file ]&&[ $file ~= ${file//InverseWarp} ]&& echo -n \"$file \"`"
          date
          echo "--------------------------------------------------------------------------------------"
          #${ANTSPATH}/AverageImages $dim ${templatename}${whichtemplate}warp.nii.gz 0 `ls ${outputname}*Warp.nii.gz | grep -v "InverseWarp"`
          ${ANTSPATH}/AverageImages $dim ${templatename}${whichtemplate}warp.nii.gz 0 `for file in ${outputname}*Warp.nii.gz ; do [ -s $file ]&&[ $file ~= ${file//InverseWarp} ]&& echo -n "$file "` ;
        ...
        #echo "   ${ANTSPATH}/${AVERAGE_AFFINE_PROGRAM} ${dim} ${templatename}0Affine.mat ${outputname}*Affine.mat"
        #echo "   ${ANTSPATH}/${AVERAGE_AFFINE_PROGRAM} ${dim} ${templatename}Affine.mat ${outputname}*Affine.mat"
#This failed because it's looking for more than one affine (one for each channel) but really, there's just one because we're using all images for cost to produce one transform
        #echo "   ${ANTSPATH}/${AVERAGE_AFFINE_PROGRAM} ${dim} ${templatename}Affine.txt ${outputname}*Affine.txt"
        echo "   ${ANTSPATH}/${AVERAGE_AFFINE_PROGRAM} ${dim} ${templatename}Affine.txt `for file in ${outputname}*Affine.txt; do [ -s $file ]&&echo -n "$file " ; done` "
        ...
        echo "   ${WARP} -d ${dim} -e vector -i ${templatename}0warp.nii.gz -o ${templatename}0warp.nii.gz -t [${templatename}Affine.txt,1] -r ${template} --verbose 1"
        echo "--------------------------------------------------------------------------------------"

        #${ANTSPATH}/${AVERAGE_AFFINE_PROGRAM} ${dim} ${templatename}0GenericAffine.mat ${outputname}*GenericAffine.mat
        #${ANTSPATH}/${AVERAGE_AFFINE_PROGRAM} ${dim} ${templatename}Affine.txt ${outputname}*Affine.txt
        ${ANTSPATH}/${AVERAGE_AFFINE_PROGRAM} ${dim} ${templatename}Affine.txt `for file in ${outputname}*Affine.txt; do [ -s $file ]&&echo -n "$file " ; done` ;
        ...
        if [[ $NWARPS -ne 0 ]];
          then
            ...
            ${WARP} -d ${dim} -e vector -i ${templatename}0warp.nii.gz -o ${templatename}0warp.nii.gz -t [${templatename}Affine.txt,1] -r ${template} --verbose 1
    ...
    echo "   ${WARP} -d ${dim} --float $USEFLOAT --verbose 1 -i ${template} -o ${template} -t [${templatename}Affine.txt,1] -t ${templatename}0warp.nii.gz -t ${templatename}0warp.nii.gz -t ${templatename}0warp.nii.gz -t ${templatename}0warp.nii.gz -r ${template}"
    echo "--------------------------------------------------------------------------------------"
    ...
    if [ -f "${templatename}0warp.nii.gz" ];
      then
        ...
        ${WARP} -d ${dim} --float $USEFLOAT --verbose 1 -i ${template} -o ${template} -t [${templatename}Affine.txt,1] -t ${templatename}0warp.nii.gz -t ${templatename}0warp.nii.gz -t ${templatename}0warp.nii.gz -t ${templatename}0warp.nii.gz -r ${template}
      else
        #${WARP} -d ${dim} --float $USEFLOAT --verbose 1 -i ${template} -o ${template} -t [${templatename}0GenericAffine.mat,1] -r ${template}
        ${WARP} -d ${dim} --float $USEFLOAT --verbose 1 -i ${template} -o ${template} -t [${templatename}Affine.txt,1] -r ${template}
      fi
...
while getopts "a:b:c:d:e:f:g:h:i:j:k:l:m:n:o:p:q:s:r:t:u:v:w:x:y:z:" OPT
  ...
  case $OPT in
   ...
   ;;
      c) #use SGE cluster
   DOQSUB=$OPTARG
#BMN increase to 6 and add condor as 6 now instead of 5 :-(
   if [[ $DOQSUB -gt 6 ]];
     then
       echo " DOQSUB must be an integer value (0=serial, 1=SGE qsub, 2=try pexec, 3=XGrid, 4=PBS qsub, 5=SLURM, 6=CONDOR) you passed  -c $DOQSUB "
       exit 1
     fi
   ;;
      d) #dimensions
...
#Put a timestamp here so can figure out when program was started
date
...
#BMN add a check to make sure you have access to CONDOR
if [[ $DOQSUB -eq 6 ]]; then
    qq=`which condor_q`
    if [[ ${#qq} -lt 1 ]]; then
        echo "do you have condor_q?  if not, then choose another c option ... if so, then check where the sbatch alias points ..."
        exit
    fi
fi
...
if [[ "$RIGID" -eq 1 ]];
  then
	...
	#BMN CONDOR CHANGE
#        basecall="${ANTS} -d ${DIM} --use-all-metrics-for-convergence --float $USEFLOAT --verbose 1 -u 1 -w [0.01,0.99] -z 1 -r [${TEMPLATES[0]},${IMAGESETARRAY[$i]},1]"
        basecall="ANTS ${DIM} --use-all-metrics-for-convergence --verbose ";
	#BMN CONDOR CHANGE
	...
	#BMN
	#SKIP THIS FOR CONDOR SINCE CANNOT USE antsRegistration
#        exe="${basecall} ${stage1}"
	#/BMN

        qscript="${outdir}/job_CHTC_${count}_qsub.sh"
        rm -f $qscript

	#BMN let's include this for CONDOR
        #if [[ $DOQSUB -eq 5 ]];
        if [[ $DOQSUB -gt 4 ]];
            then
            # SLURM job scripts must start with a shebang
#            echo '#!/bin/sh' > $qscript
	    #BMN Xiao specified this for the larger CONDOR net
	    echo '#!/usr/bin/env bash' >> $qscript 	
	    #BMN one of these should work, huh?
            fi
	...
	#BMN Need to be really clear what input images need to transfer to complete this particular job
	IMAGEcondor=""
	TEMPLATEcondor=""
        for (( j = 0; j < $NUMBEROFMODALITIES; j++ ))
          do
	...
	#BMN
            #IMAGEcondor="$IMAGEcondor, ./${IMAGESETARRAY[$k]}"; #BMN I'm modifying this to just use a spaced string and implicit split instead of an array/hash
            IMAGEcondor="$IMAGEcondor, "`basename ${IMAGESETARRAY[$k]}`; #BMN don't need the ./
	    #This was messing everything up in the submit file because when it tried to transfer back it had the whole path
            #TEMPLATEcondor="$TEMPLATEcondor, ./${TEMPLATES[$k]}" ; #BMN again making a list of the templates for each modality so can transfer them
            #TEMPLATEcondor="$TEMPLATEcondor, ${TEMPLATES[$k]}" ; #BMN again making a list of the templates for each modality so can transfer them
            TEMPLATEcondor="$TEMPLATEcondor, "`basename ${TEMPLATES[$k]}` ; #BMN again making a list of the templates for each modality so can transfer them
	    echo "DEBUG: IMAGESETARRAY[k] is ${IMAGESETARRAY[$k]} " ;
	    echo "DEBUG: TEMPLATES[k] is ${TEMPLATES[$k]} " ;
	#/BMN
	...
	#BMN Get Rid of This Too for CONDOR
	...
	echo "DEBUG DOQSUB IS $DOQSUB";
        ...
        if [[ $DOQSUB -eq 1 ]];
          ...
          then
            ...
            jobIDs="$jobIDs $id"
            sleep 0.5
	#BMN NOW ADD CONDOR TO THIS
	elif [ $DOQSUB -eq 6 ]; 
	    then
	    echo "You chose to run on CONDOR.  Make sure this computer is a submit node!" ;
	    #BMN YES WE NEED THE ./ HERE NOW
	    echo "chmod +x ./$ANTS ./$WARP ./$N4 ./$PEXEC " >> $qscript ;
	    echo "export PATH=$PWD:$PATH" >> $qscript ;
	    chmod +x $qscript ; ###THIS IS ESSENTIAL FOR CONDOR!
	#Xiaojue had to add these for transferring back 
            #echo "mkdir ${outdir}" >> $qcondorscript
	#This means somewhere it is taking paths literally when they should be local/relative
	#Keeping in mind that I had several issues with the call to create a temporary directory as subfolder instead of calling the system tempdir command (either way we have to clean it up), this could likely be fixed by finding the right reference in the script(s)
	#she also prepends this:
            #echo '#!/usr/bin/env bash' > $qcondorscript
	#which I have now added in the appropriate location above
            #HER COMMAND:
	    #echo -e "Universe = vanilla\ngetenv = True\nArguments = \nPeriodicRemove =  (((CurrentTime - EnteredCurrentStatus) > (2*24*3600)) && JobStatus == 5)\n\ninitialdir = $PWD\nExecutable = $qcondorscript \nshould_transfer_files = yes \nwhen_to_transfer_output = ON_EXIT \ntransfer_input_files = /StudyDir/bin/ANTS,/StudyDir/bin/ImageMath,/StudyDir/bin/N4BiasFieldCorrection,/StudyDir/bin/WarpImageMultiTransform, /StudyDir/bin/ANTSpexec.sh, $IMAGEcondor, $TEMPLATEcondor \nrequest_disk = 250MB\nrequest_memory = 8GB\nLog = ${qcondorscript}.log\nError = ${qcondorscript}.err\nQueue\n" > ${qcondorscript}.submit;
            #echo -e "Universe = vanilla\ngetenv = True\nArguments = \nPeriodicRemove =  (((CurrentTime - EnteredCurrentStatus) > (2*24*3600)) && JobStatus == 5)\n\ninitialdir = ./\nExecutable = $qscript \nshould_transfer_files = yes \nwhen_to_transfer_output = ON_EXIT \ntransfer_input_files = ./$ANTS, ./$WARP, ./$N4, ./$PEXEC, ./$COND, ./$IMAGEcondor, ./$TEMPLATEcondor \nrequest_disk = 30GB\nrequest_memory = ${MEMORY}\nLog = ${qscript}.log\nError = ${qscript}.err\nQueue\n " > ${qscript}.submit;
	    #BMN I'm ditching the ./
            echo -e "Universe = vanilla\ngetenv = True\nArguments = \nPeriodicRemove =  (((CurrentTime - EnteredCurrentStatus) > (2*24*3600)) && JobStatus == 5)\n\ninitialdir = \nExecutable = $qscript \nshould_transfer_files = yes \nwhen_to_transfer_output = ON_EXIT \ntransfer_input_files = ./$ANTS, ./$WARP, ./$N4, ./$PEXEC, ./$IMAGEcondor, ./$TEMPLATEcondor \nrequest_disk = 30GB\nrequest_memory = ${MEMORY}\nLog = ${qscript}.log\nError = ${qscript}.err\nOutput = ${qscript}.output\nQueue\n " > ${qscript}.submit;
	    echo "Debug, qscript is $qscript and contains" `cat $qscript` ;
	    echo "Debug, qscript is ${qscript}.submit and contains" `cat ${qscript}.submit` ;

	#BMN note that I'm ditching the $PWD, since I expect that's what causes the need to mkdir ${outdir} which is $PWD unless otherwise specified.  I also will change the paths so we can actually use the script for general purpose ; as far as I can tell, DEFORMED needs to be an array of 3 images, so we really need to specify that well
	#BMN here's my old command
#	    echo -e "Universe = vanilla\ngetenv = True\nArguments = \nPeriodicRemove =  (((CurrentTime - EnteredCurrentStatus) > (2*24*3600)) && JobStatus == 5)\n\ninitialdir = $PWD\nExecutable = $qscript \nrequest_disk = 250MB\nrequest_memory = 8GB\nLog = ${qscript}.log\nError = ${qscript}.err\nQueue\n" > ${qscript}.submit;
	     id=`condor_submit ${qscript}.submit | awk '{print $6}'`0 ;
	     echo "Debug: adding job ID $id"
	     jobIDs="$jobIDs $id"
	#BMN END OF NEW BLOCK
    ...
    #BMN NOW DO A BLOCK WHERE WE WAIT FOR JOBS TO FINISH
    if [ $DOQSUB -eq 6 ];
      then
      echo
      echo "--------------------------------------------------------------------------------------"
      echo " Starting ANTS registration on CONDOR. Submitted $count jobs " 
      echo "--------------------------------------------------------------------------------------"
#NOTE WE WILL HAVE TO PUT THIS IN ANTSPATH NOT JUST CURRENT DIRECTORY
	./waitForCONDORJobs_wrapper.pl 1 120 $jobIDs
      if [ ! $? -eq 0 ]; then
        echo "CONDOR submission failed - jobs went into error state"
        exit 1;
      fi
    fi
    #BMN END OF NEW BLOCK
    ...
    for (( j = 0; j < $NUMBEROFMODALITIES; j++ ))
      ...
      echo "DEBUG: summarizeimageset $DIM ${TEMPLATES[$j]} $STATSMETHOD ${IMAGERIGIDSET[@]}" 
    ...
    if [[ BACKUPEACHITERATION -eq 1 ]];
      then
        ...
        mv ${outdir}/rigid*.nii.gz ${outdir}/*Affine.txt ${outdir}/rigid/
        # backup logs
        if [[ $DOQSUB -eq 1 ]];
          then
          ...
          then
            ...
            rm -f ${outdir}/job_${count}_qsub.sh
	#BMN JUST COPY EVERYTHING FOR NOW
        elif [[ $DOQSUB -eq 6 ]];
          then
            mv ${outdir}/job*sh ${outdir}/rigid/
            mv ${outdir}/job*.err ${outdir}/rigid/
            mv ${outdir}/job*.submit ${outdir}/rigid/
            mv ${outdir}/job*.log ${outdir}/rigid/
	#BMN END OF NEW BLOCK
        fi
      else
...
#    rm -f ${outdir}/job*.sh
    ...
    for (( j = 0; j < ${#IMAGESETARRAY[@]}; j+=$NUMBEROFMODALITIES ))
      do
	#BMN
        #basecall="${ANTS} -d ${DIM} --use-all-metrics-for-convergence --float $USEFLOAT --verbose 1 -u 1 -w [0.01,0.99] -z 1"
        basecall="ANTS ${DIM} --use-all-metrics-for-convergence --verbose ";
	#/BMN
        ...
        pexe=''
        warppexe=''
	#BMN initialize so can build a string
	DEFORMEDcondor='';
	IMAGEcondor='';
	TEMPLATEcondor='';
	#BMN
	...
	#BMN
            TEMPLATEcondor="$TEMPLATEcondor,$PWD/${TEMPLATES[$k]}" ; #BMN again making a list of the templates for each modality so can transfer them
	    echo "DEBUG: TEMPLATES[k] is ${TEMPLATES[$k]} " ;
	    echo "DEBUGL so TEMPLATEcondor is $TEMPLATEcondor";
	#/BMN
            indir=`dirname ${IMAGESETARRAY[$j]}`
	#BMN
            IMAGEcondor="$IMAGEcondor,$PWD/${IMAGESETARRAY[$l]}"; #BMN I'm modifying this to just use a spaced string and implicit split instead of an array/hash
	    echo "DEBUG: IMAGESETARRAY[l] is ${IMAGESETARRAY[$l]} " ;
	    echo "DEBUGL so IMAGEcondor is $IMAGEcondor";
	#/BMN
            if [[ ${#indir} -eq 0 ]];
              then
                indir=`pwd`
	    ...
	    #Gotta get rid of all this outdir crap
            DEFORMED="${OUTFN}${l}WarpedToTemplate.nii.gz"
	    #BMN let's make sure everything is comma-separated and references just the local directory so it can run in a condor tempdir
	    #DEFORMEDcondor="$DEFORMEDcondor,$PWD/${OUTFN}${l}WarpedToTemplate.nii.gz"; #This collects the outputs for this job I hope
	    DEFORMEDcondor="$DEFORMEDcondor,${OUTFN}${l}WarpedToTemplate.nii.gz"; #This collects the outputs for this job I hope
	    #BMN then again the l=j+k so I'm not toally sure we're indexing into the right thing here if it's purely numerical
            ...
            if [ $NOWARP -eq 0 ];
              then
		#NOTE, THIS HAD TO BE CHANGED FROM 1, MAYBE BECAUSE WE'RE USING THE --use-all-metrics-for-convergence ?
                #OUTPUTTRANSFORMS="-t ${outdir}/${OUTWARPFN}1Warp.nii.gz -t ${outdir}/${OUTWARPFN}0GenericAffine.mat"
                #OUTPUTTRANSFORMS="-t ${outdir}/${OUTWARPFN}Warp.nii.gz -t ${outdir}/${OUTWARPFN}Affine.txt"
		#BMN no path!!
                OUTPUTTRANSFORMS="-t ${OUTWARPFN}Warp.nii.gz -t ${OUTWARPFN}Affine.txt"
              else
                #OUTPUTTRANSFORMS="-t ${outdir}/${OUTWARPFN}0GenericAffine.mat"
                #OUTPUTTRANSFORMS="-t ${outdir}/${OUTWARPFN}Affine.txt"
                OUTPUTTRANSFORMS="-t ${OUTWARPFN}Affine.txt"
              fi
		...
		#NOPE, no path!
                REPAIRED="${OUTFN}Repaired.nii.gz"
		#BMN put martin's option to use all metrics for the cost so we really use the other channels
		#BMN Note
		#ANTS 3 -m "CC[1_t1w.nii.gz,2_t1w.nii.gz,1,2]" -m "CC[1_t2w.nii.gz,2_t2w.nii.gz,1,2]" -i 100x30x20 -o 1x2Total.nii.gz     -t 'SyN[0.125]' -r 'Gauss[2,0]' --use-all-metrics-for-convergence is martin's
		#BMN Note
		if [ $((NUMBEROFMODALITIES)) -eq 1 ] ; then
			...
			exe=" $exe $N4 -d ${DIM} -b [200] -c [50x50x40x30,0.00000001] -i ${IMAGESETARRAY[$l]} -o ${REPAIRED} -r 0 -s 2 --verbose 1\n"
			pexe=" $pexe $N4 -d ${DIM} -b [200] -c [50x50x40x30,0.00000001] -i ${IMAGESETARRAY[$l]} -o ${REPAIRED} -r 0 -s 2 --verbose 1  >> ${outdir}/job_${count}_metriclog.txt >> ${outdir}/job_${count}_metriclog.txt\n"
			#exe=" $exe $N4 -d ${DIM} -b [200] -c [50x50x40x30,0.00000001] -i ${IMAGESETARRAY[$l]} -o ${REPAIRED} -r 0 -s 2 --verbose 1 --use-all-metrics-for-convergence\n"
			#pexe=" $pexe $N4 -d ${DIM} -b [200] -c [50x50x40x30,0.00000001] -i ${IMAGESETARRAY[$l]} -o ${REPAIRED} -r 0 -s 2 --verbose 1  --use-all-metrics-for-convergence >> ${outdir}/job_${count}_metriclog.txt >> ${outdir}/job_${count}_metriclog.txt\n"
		fi
		#\BMN

                #IMAGEMETRICSET="$IMAGEMETRICSET -m '${METRIC}${TEMPLATES[$k]},${REPAIRED},${METRICPARAMS}'"
                #IMAGEMETRICSET="$IMAGEMETRICSET -m '${METRIC}${PWD}/${TEMPLATES[$k]},${PWD}/${REPAIRED},${METRICPARAMS}'"
                #IMAGEMETRICSET="$IMAGEMETRICSET -m '${METRIC}./${TEMPLATES[$k]},./${REPAIRED},${METRICPARAMS}'"
		#BMN don't need the ./
                IMAGEMETRICSET="$IMAGEMETRICSET -m '${METRIC}${TEMPLATES[$k]},${REPAIRED},${METRICPARAMS}'"
                ...
                #IMAGEMETRICSET="$IMAGEMETRICSET -m '${METRIC}${PWD}/${TEMPLATES[$k]},${PWD}/${IMAGESETARRAY[$l]},${METRICPARAMS}'"
                #IMAGEMETRICSET="$IMAGEMETRICSET -m '${METRIC}./${TEMPLATES[$k]},./${IMAGESETARRAY[$l]},${METRICPARAMS}'"
		#BMN don't need ./
                IMAGEMETRICSET="$IMAGEMETRICSET -m '${METRIC}${TEMPLATES[$k]},${IMAGESETARRAY[$l]},${METRICPARAMS}'"
	...
	#BMN This runs
	# ANTS 3 --use-all-metrics-for-convergence --verbose -t 'SyN[0.1,3,0]' -m CC[template_SY_ALL14_2chantemplate0.nii.gz,s205abcd_axbig_resamp_N4_ave_hyperedge.nii.gz,1,4] -m CC[template_SY_ALL14_2chantemplate1.nii.gz,s205abcd_axbig_resamp_N4_ave_Squared_hyperedge.nii.gz,1,4] -i '[100x100x70x20,1e-9,10]' -r 'GAUSS[3,2,1,0]' -o ./template_SY_ALL14_2chans205abcd_axbig_resamp_N4_ave_hyperedge0 -a ./template_SY_ALL14_2chans205abcd_axbig_resamp_N4_ave_hyperedge00GenericAffine.mat
	#This is what we're up against: -t SyN[0.1,3,    0]  -m CC[template_SY_ALL14_2chantemplate0.nii.gz,s205abcd_axbig_resamp_N4_ave_hyperedge.nii.gz,1,4] -m CC[template_SY_ALL14_2chantemplate1.nii.gz,s205abcd_axbig_resamp_N4_ave_Squared_hyperedge.nii.gz,1,4] -c [100x100x70x20,1e-9,10] -f 6x4x2x1 -s 3x2x1x0 -o ./templat    e_SY_ALL14_2chans205abcd_axbig_resamp_N4_ave_hyperedge0
	#
        #stage3="-t ${TRANSFORMATION} ${IMAGEMETRICSET} -c [${MAXITERATIONS},1e-9,10] -f ${SHRINKFACTORS} -s ${SMOOTHINGFACTORS} -o ${outdir}/${OUTWARPFN}"
#Still need to figure out what to map the shrink factor onto for the actual ANTS call
#        stage3="-t '${TRANSFORMATION}' ${IMAGEMETRICSET} -i [${MAXITERATIONS},1e-9,10] -r 'GAUSS[${SMOOTHINGFACTORS}]' -o ${outdir}/${OUTWARPFN} -a ${outdir}/${OUTWARPFN}*GenericAffine.mat"
	#if [ -s ${outdir}/${OUTWARPFN}*GenericAffine.mat ] ; then 
	if [ -s ${outdir}/${OUTWARPFN}*Affine.txt ] ; then 
		#stage3="-t '${TRANSFORMATION}' ${IMAGEMETRICSET} -i [${MAXITERATIONS},1e-9,10] -r 'GAUSS[${SMOOTHINGFACTORS}]' -o ${outdir}/${OUTWARPFN} -a ${outdir}/${OUTWARPFN}*GenericAffine.mat"
		#stage3="-t '${TRANSFORMATION}' ${IMAGEMETRICSET} -i [${MAXITERATIONS},1e-9,10] -r 'GAUSS[${SMOOTHINGFACTORS}]' -o ${PWD}/${OUTWARPFN} -a ${outdir}/${OUTWARPFN}*GenericAffine.mat"
		#stage3="-t '${TRANSFORMATION}' ${IMAGEMETRICSET} -i [${MAXITERATIONS},1e-9,10] -r 'GAUSS[${SMOOTHINGFACTORS}]' -o ${PWD}/${OUTWARPFN} -a ${outdir}/${OUTWARPFN}*Affine.txt"
		#stage3="-t '${TRANSFORMATION}' ${IMAGEMETRICSET} -i ${MAXITERATIONS} -r 'GAUSS[${SMOOTHINGFACTORS}]' -o ${PWD}/${OUTWARPFN} -a ${outdir}/${OUTWARPFN}*Affine.txt"
#		stage3="-t '${TRANSFORMATION}' ${IMAGEMETRICSET} -i ${MAXITERATIONS} -r 'GAUSS[${SMOOTHINGFACTORS}]' -o ${OUTWARPFN} -a ${outdir}/${OUTWARPFN}*Affine.txt"
		#NOPE, GOT TO GET RID OF OUTDIR and then we have to make sure that this gets transferred
		stage3="-t '${TRANSFORMATION}' ${IMAGEMETRICSET} -i ${MAXITERATIONS} -r 'GAUSS[${SMOOTHINGFACTORS}]' -o ${OUTWARPFN} -a ${OUTWARPFN}*Affine.txt"
	else 
		#stage3="-t '${TRANSFORMATION}' ${IMAGEMETRICSET} -i [${MAXITERATIONS},1e-9,10] -r 'GAUSS[${SMOOTHINGFACTORS}]' -o ${outdir}/${OUTWARPFN} " ;
		#stage3="-t '${TRANSFORMATION}' ${IMAGEMETRICSET} -i [${MAXITERATIONS},1e-9,10] -r 'GAUSS[${SMOOTHINGFACTORS}]' -o ${PWD}/${OUTWARPFN} " ;
		#stage3="-t '${TRANSFORMATION}' ${IMAGEMETRICSET} -i ${MAXITERATIONS} -r 'GAUSS[${SMOOTHINGFACTORS}]' -o ${PWD}/${OUTWARPFN} " ;
		#OOPS! This was causing all the jobs to fail because it couldn't write this because I hadn't escaped out or single-quoted the ${PWD}; 
		#Omitting completely works fine
		stage3="-t '${TRANSFORMATION}' ${IMAGEMETRICSET} -i ${MAXITERATIONS} -r 'GAUSS[${SMOOTHINGFACTORS}]' -o ${OUTWARPFN} " ;
	fi
	#/BMN note that the GenericAffine.mat may or may not exist at this point (we'll see if we can stage the rigid->affine with real ANTS calls later
	    ...
	    #/BMN
		  ...
		  exe="$exebase ${basecall} --rigid-affine ${stage3}\n";
		  pexe="$pexebase ${basecall} --rigid-affine ${stage3} >> ${outdir}/job_${count}_metriclog.txt\n"
	        else
		  ...
		  exe="$exebase ${basecall} --do-rigid ${stage3}\n";
		  pexe="$pexebase ${basecall} --do-rigid ${stage3} >> ${outdir}/job_${count}_metriclog.txt\n"
                fi
          else
	         #BMN at this point, we're just going to run the warp and trust the implicit early stages
        ...
        qscript="${outdir}/job_CHTC_${count}_${i}.sh"

        echo -e $exe >> ${outdir}/job_${count}_${i}_metriclog.txt
#BMN ADDING CONDOR
        # 6 submit to SGE (DOQSUB=1), PBS (DOQSUB=4), PEXEC (DOQSUB=2), XGrid (DOQSUB=3), SLURM (DOQSUB=5) , CONDOR (DOQSUB=6) or else run locally (DOQSUB=0)
        if [[ $DOQSUB -eq 1 ]];
          then
            echo "$SCRIPTPREPEND" > $qscript
            echo -e "$exe" >> $qscript
          ...
          then
            ...
            jobIDs="$jobIDs $id"
            sleep 0.5
	#BMN ADD CONDOR EXECUTE AND GRAB JOB IDS
    	elif [ $DOQSUB -eq 6 ]; 
	  then
            echo '#! /usr/bin/env bash' > $qscript #better to use this than #! /bin/bash
	    echo -e "$SCRIPTPREPEND" >> $qscript 
	    #echo "chmod +x ./$ANTS ./$WARP ./$N4 ./$PEXEC " >> $qscript ;
	    echo 'chmod +x $PWD/'$ANTS' $PWD/'$WARP' $PWD/'$N4' $PWD/'$PEXEC' ' >> $qscript ;
	    echo 'export PATH=$PWD:$PATH' >> $qscript ;
	    echo -e "$exe" >> $qscript
	    #echo "$exe2" >> $qscript
	    chmod +x $qscript ; ###THIS IS ESSENTIAL FOR CONDOR!
	    #BMN this is the old command 
	    #echo -e "Universe = vanilla\ngetenv = True\nArguments = \nPeriodicRemove =  (((CurrentTime - EnteredCurrentStatus) > (2*24*3600)) && JobStatus == 5)\n\ninitialdir = $PWD\nExecutable = $qscript \nrequest_disk = 250MB\nrequest_memory = 8GB\nLog = ${qscript}.log\nError = ${qscript}.err\nQueue\n" > ${qscript}.submit;
	#Taking a pass at Xiaojue's script but generalizing to use relative paths:
            #echo -e "Universe = vanilla\ngetenv = True\nRequirements  = ( OpSys == \"LINUX\" && Arch == \"X86_64\" )\nArguments = \nPeriodicRemove =  (((CurrentTime - EnteredCurrentStatus) > (2*24*3600)) && JobStatus == 5)\n\ninitialdir = $PWD\nExecutable = $qcondorscript \nshould_transfer_files = yes \nwhen_to_transfer_output = ON_EXIT \ntransfer_input_files = /StudyDir/bin/ANTS,/StudyDir/bin/ImageMath,/StudyDir/bin/N4BiasFieldCorrection,/StudyDir/bin/WarpImageMultiTransform,/StudyDir/bin/ANTSpexec.sh,$IMAGEcondor,$TEMPLATEcondor \ntransfer_output_files = ${OUTWARPFN}Warp.nii.gz,${OUTWARPFN}Affine.txt,${OUTWARPFN}InverseWarp.nii.gz,${DEFORMEDCondor} \nrequest_disk = 50MB\nrequest_memory = 2GB\nLog = ${qcondorscript}.log\nError = ${qcondorscript}.err\nQueue\n" > ${qcondorscript}.submit;
#note, DON'T put $PWD before deformed condor since this is a list of deformed outputs that already have $PWD prepended
            #echo -e "Universe = vanilla\ngetenv = True\nRequirements  = ( OpSys == \"LINUX\" && Arch == \"X86_64\" )\nArguments = \nPeriodicRemove =  (((CurrentTime - EnteredCurrentStatus) > (2*24*3600)) && JobStatus == 5)\n\ninitialdir = $PWD/\nExecutable = $qscript \nshould_transfer_files = yes \nwhen_to_transfer_output = ON_EXIT \ntransfer_input_files = $PWD/$ANTS, $PWD/$WARP, $PWD/$N4, $PWD/$PEXEC, $IMAGEcondor,$TEMPLATEcondor \ntransfer_output_files = $PWD/${OUTWARPFN}Warp.nii.gz,$PWD/${OUTWARPFN}Affine.txt,$PWD/${OUTWARPFN}InverseWarp.nii.gz,${DEFORMEDcondor} \nrequest_disk = 30GB\nrequest_memory = ${MEMORY}\nLog = ${qscript}.log\nError = ${qscript}.err\nQueue\n" > ${qscript}.submit;
            #echo -e "Universe = vanilla\ngetenv = True\nRequirements  = ( OpSys == \"LINUX\" && Arch == \"X86_64\" )\nArguments = \nPeriodicRemove =  (((CurrentTime - EnteredCurrentStatus) > (2*24*3600)) && JobStatus == 5)\n\ninitialdir = $PWD/\nExecutable = $qscript \nshould_transfer_files = yes \nwhen_to_transfer_output = ON_EXIT \ntransfer_input_files = $PWD/$ANTS, $PWD/$WARP, $PWD/$N4, $PWD/$PEXEC, $IMAGEcondor,$TEMPLATEcondor \ntransfer_output_files = ${OUTWARPFN}Warp.nii.gz,${OUTWARPFN}Affine.txt,${OUTWARPFN}InverseWarp.nii.gz,${DEFORMEDcondor} \nrequest_disk = 30GB\nrequest_memory = ${MEMORY}\nLog = ${qscript}.log\nError = ${qscript}.err\nQueue\n" > ${qscript}.submit;
	    #Since we might call the ${OUTWARPFN}Affine.txt:
            echo -e "Universe = vanilla\ngetenv = True\nRequirements  = ( OpSys == \"LINUX\" && Arch == \"X86_64\" )\nArguments = \nPeriodicRemove =  (((CurrentTime - EnteredCurrentStatus) > (2*24*3600)) && JobStatus == 5)\n\ninitialdir = $PWD/\nExecutable = $qscript \nshould_transfer_files = yes \nwhen_to_transfer_output = ON_EXIT \ntransfer_input_files = $PWD/$ANTS, $PWD/$WARP, $PWD/$N4, $PWD/$PEXEC, $IMAGEcondor,$TEMPLATEcondor, ${OUTWARPFN}Affine.txt \ntransfer_output_files = ${OUTWARPFN}Warp.nii.gz,${OUTWARPFN}Affine.txt,${OUTWARPFN}InverseWarp.nii.gz,${DEFORMEDcondor} \nrequest_disk = 30GB\nrequest_memory = ${MEMORY}\nLog = ${qscript}.log\nError = ${qscript}.err\nOutput = ${qscript}.output\nQueue\n" > ${qscript}_${OUTWARPFN}.submit;
	    echo "Debug, qscript is $qscript and contains" `cat $qscript` ;
	    echo "Debug, qscript is ${qscript}_${OUTWARPFN}.submit and contains" `cat ${qscript}_${OUTWARPFN}.submit` ;
	    id=`condor_submit ${qscript}.submit | awk '{print $6}'`0 ;
	    echo "Debug: adding job ID $id"
	    jobIDs="$jobIDs $id"
	#BMN END OF NEW BLOCK
...
#BMN ADD CONDOR WAIT FOR JOBS
     if [ $DOQSUB -eq 6 ];
      then
      # Run jobs on CONDOR and wait for finish
      echo
      echo "--------------------------------------------------------------------------------------"
      echo " Starting ANTS registration on CONDOR. Iteration: $itdisplay of $ITERATIONLIMIT"
      echo "--------------------------------------------------------------------------------------"
...
#BMN END OF NEW BLOCK
    ...
    AFFINEFILES=`ls ${OUTPUTNAME}*Affine.txt`
    ...
    for (( j = 0; j < $NUMBEROFMODALITIES; j++ ))
      do
	echo "DEBUG: I'm about to run shapeupdatetotempalte on template ${TEMPLATES[$j]} "
	echo "This will take some troubleshooting since it's expecting to make a few of these instead of just one across the modalities"
	...
	#BMN JUST COPY EVERYTHING FOR NOW
          ...
          then
            mv ${outdir}/job*sh ${outdir}/ANTs_iteration_${i}
            mv ${outdir}/job*.err ${outdir}/ANTs_iteration_${i}
            mv ${outdir}/job*.submit ${outdir}/ANTs_iteration_${i}
            mv ${outdir}/job*.log ${outdir}/ANTs_iteration_${i}
        fi
      else

...
if [[ "$RIGID" -eq 1 ]];
  then
	...
	#BMN NOW ADD CONDOR TO THIS
	    ...
	    echo -e "Universe = vanilla\ngetenv = True\nArguments = \nPeriodicRemove =  (((CurrentTime - EnteredCurrentStatus) > (2*24*3600)) && JobStatus == 5)\n\ninitialdir = $PWD\nExecutable = $qscript \nshould_transfer_files = yes \nwhen_to_transfer_output = ON_EXIT \ntransfer_input_files = /StudyDir/bin/ANTS,/StudyDir/bin/ImageMath,/StudyDir/bin/N4BiasFieldCorrection,/StudyDir/bin/WarpImageMultiTransform, /StudyDir/bin/ANTSpexec.sh \nrequest_disk = 250MB\nrequest_memory = 8GB\nLog = ${qscript}.log\nError = ${qscript}.err\nQueue\n" > ${qscript}.submit;
